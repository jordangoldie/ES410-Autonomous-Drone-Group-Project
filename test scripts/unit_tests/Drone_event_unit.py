...
import threading
from GPS_unit import get_vector, get_gps, distance_between
import numpy as np
...
class Drone:
    def __init__(self, connection_str):
        try:
            self.vehicle = connect(connection_str)               # return vehicle
            # create events
            ...
            self.waypoint_count = 0              # waypoint count
            self.origin = np.array               # create array for origin
    ...
    def arm_and_takeoff(self, target_alt):
        # check target altitude is within range
        if target_alt <= 1:
            target_alt = 1
        elif target_alt >= 50:
            target_alt = 50
    ...
    def fly_to_point(self, location, airspeed, plant_flag):
        ...
        self.vehicle.simple_goto(location)            # command drone to travel to location

        # keep drone flying to location until within 1 m
        while True:
    ...
    def descend(self, target_alt):
        ...
        while True:
            ...
            if self.vehicle.location.global_relative_frame.alt <= target_alt * 1.02:

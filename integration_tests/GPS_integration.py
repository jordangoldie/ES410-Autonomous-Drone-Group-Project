...
def normalise(v): # pragma:  no cover
    v = v/npl.norm(v)
    return v


# gets the global cartesian coordinates (ECEF) of a gps point
def gps_to_cartesian(lat, long): # pragma:  no cover
    #lat = int
    #long = int
    radius = 6378100
    # degrees to radians:
...
def cartesian_to_gps(global_vec): # pragma:  no cover
    radius = 6378100
...
def get_gps(local_frame, vec_local): # pragma:  no cover
...
def get_circle_coords(lat, long, origin): # pragma:  no cover
    radius = 2
